...
sudo chroot /opt/chroots/debian_jessie_x32 /bin/bash -c "rm -f /root/jenkins/workspace/sfml-sidescroller_x32/build/bomberpi*"
sudo chroot /opt/chroots/debian_jessie_x32 /bin/bash -c "cd /root/jenkins/workspace/sfml-sidescroller_x32/build && cmake .. -DCMAKE_PREFIX_PATH=/usr/local/lib/cmake -DBOMBERPI_VERSION_PATCH="\""$(cat /var/lib/jenkins/jobs/sfml-sidescroller_x32/nextBuildNumber)"\"
